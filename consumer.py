...
class ExampleConsumer(object):
    ...
    def __init__(self, amqp_url, ioloop=None):
    ...
    def connect(self):
        ...
        if self._ioloop:
            self._connection = pika.adapters.TornadoConnection(pika.URLParameters(self._url), on_open_callback=self.on_connection_open)
        else:
            self._connection = pika.SelectConnection(pika.URLParameters(self._url), self.on_connection_open)
...
class PikaClient(ExampleConsumer):
    event_listeners = set()

    def on_message(self, unused_channel, basic_deliver, properties, body):
        logging.info('PikaClient: message received: %s' % body)
        self.notify_listeners(body)
        super(PikaClient, self).on_message(unused_channel, basic_deliver, properties, body)
    ...
    def add_event_listener(self, listener):
        ...
        listener.write_message('PikaClient: listener %s added' % repr(listener))
